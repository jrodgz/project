...
(function(predefined, d3, $, drawing, preprocess, undefined) {
   ...
   predefined.DynamicTextNodeView = function(g, w, h) {
      ...
      this.setCollections = function() {
         if (collectionNodes == null) {
            collectionNodes = [];
            var collections = preprocess.getCollections();
            for (key in collections) {
               collectionNodes.push({
                  text: key,
                  style: defaultTextStyle,
                  payload: collections[key],
                  group: collections[key].length,
                  offset: drawing.visualLength(key, fontSize, font),
                  color: colorScale(key)
               });
            }
         }
         nodeCanvas.setData(collectionNodes, 'i');
      };

      this.setArchivers = function() {
         if (archiverNodes == null) {
            archiverNodes = [];
            var archivers = preprocess.getArchivers();
            for (key in archivers) {
               archiverNodes.push({
                  text: key,
                  style: defaultTextStyle,
                  payload: archivers[key],
                  group: archivers[key].length,
                  offset: drawing.visualLength(key, fontSize, font),
                  color: colorScale(preprocess.archiverToCollection(key))
               });
            }
         }
         nodeCanvas.setData(archiverNodes, 'i');
      };
         ...
         if (tagNodes == null) {
            ...
            for (key in tags) {
               tagNodes.push({
                  ...
                  offset: drawing.visualLength(key, fontSize, font),
                  color: colorScale(preprocess.tagToCollection(key))
               });
            }
         }
         nodeCanvas.setData(tagNodes, 'i');
      };
         ...
         if (domainNodes == null) {
            ...
            for (key in domains) {
               domainNodes.push({
                  ...
                  offset: drawing.visualLength(key, fontSize, font),
                  color: colorScale(preprocess.domainToCollection(key))
               });
            }
         }
         nodeCanvas.setData(domainNodes, 'i');
      };
...
<input class='group-by' type='radio' name='group-by' value='collections' /> collections\
<input class='group-by' type='radio' name='group-by' value='archivers' /> archivers\
<input class='group-by' type='radio' name='group-by' value='tags' /> tags\
<input class='group-by' type='radio' name='group-by' value='domains' checked='checked' /> domains\
</span>\
